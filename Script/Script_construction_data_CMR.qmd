...
table_CMR<-generate_CMR_table("dnest_max_life_class")%>%select(ID,Sexe,month,state)%>%arrange(by=month)%>%
  ...
  arrange(ID) 
write.csv(table_CMR,file=here("data","table_dnest_max_life.csv"))
```

Ici on peut visualiser les histoires de vie des individus

```{r}
generate_CMR_table("dnest_max_life_class")->table
for(ind in unique(table$ID)){
  p<-table%>%
    filter(ID==ind)%>%
    ggplot(aes(x=month,y=state))+geom_line()+
