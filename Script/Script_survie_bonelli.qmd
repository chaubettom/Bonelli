...
Les différentes colonnes de data_cox :

time=nombre de jour d'erratisme jusqu'à mort ou arrêt suivi

Date_debut= date pose

Date_fin = date fin de suivi ou mort

Trois colonnes de métriques

IMPORTANT : status correspond à une colonne qui indique si les données sont censurées (1= fin de suivi cause inconnue/perte GPS), ou correspondent à une mortalité (2).

Trois colonnes de métriques catégorielles: 4 classes de comportements en prenant comme limite les quartiles de chaque métriques : très grand (Q4), grand (Q3), moyen (Q2) et petit voyageur (Q1). Cela correspond aux colonnes max_dist_nest_class, mean_dist_nest_class, ... du fichier.
...
On regarde comment évolue les métriques au cours du temps.

DISTANCE JOURNALIERE

```{r}
for(k in 1:length(list_ltraj_bonelli)){
  ind=names(list_ltraj_bonelli)[k]
  data_ind<-list_ltraj_bonelli[[ind]]
   data_ind<- data_ind %>%
  arrange(date) %>%
  mutate(average_distance_before = lag(cumsum(dist), default = 0) / lag(row_number(), default = 1))
  if(!is.na(unique(data_ind$duration_errat))){
    if(unique(data_ind$duration_errat)>365){
    print(ggplot(data=data_ind,aes(y=average_distance_before/1000,x=date))+geom_line()+ggtitle(ind))
  }
...
for(k in 1:length(list_ltraj_bonelli)){
  ...
  if(!is.na(unique(data_ind$duration_errat))){
    if(unique(data_ind$duration_errat)>365){
    print(ggplot(data=data_ind,aes(y=cumsum(average_distance_before),x=date))+geom_line()+ggtitle(ind))
  }
...
for(k in 1:length(list_ltraj_bonelli)){
  ...
  data_ind<-list_ltraj_bonelli[[ind]]
   data_ind<- data_ind %>%
  arrange(date) %>%
  mutate(average_distnest_before = lag(cumsum(dist_nest), default = 0) / lag(row_number(), default = 1))
  if(!is.na(unique(data_ind$duration_errat))){
    if(unique(data_ind$duration_errat)>365){
    print(ggplot(data=data_ind,aes(y=average_distnest_before/1000,x=date))+geom_line()+ggtitle(ind))
  }
...
for(k in 1:length(list_ltraj_bonelli)){
  ...
  data_ind<-list_ltraj_bonelli[[ind]]
   data_ind<- data_ind %>%
  arrange(date) %>%
  mutate(max_distance_until_now = cummax(dist_nest))
  if(!is.na(unique(data_ind$duration_errat))){
    if(unique(data_ind$duration_errat)>365){
    print(ggplot(data=data_ind,aes(y=max_distance_until_now/1000,x=date))+geom_line()+ggtitle(ind))
  }
