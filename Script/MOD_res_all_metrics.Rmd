...
CMR_transition_impossible<-function(data, Last, nthin, nburnin, niter, nchain){
  ...
  my.code<-nimbleCode({
    #### PRIOR 
    for (u in 1:3){
      surv[u]~dbeta(1128,35) #probabilité de survie
    }
    transi_12 ~ dunif(0,1)
    transi_13 ~ dunif(0,1)
    transi_23 ~ dunif(0,1)
...
data_survival <- readxl::read_excel(here("Data","data_survival.xlsx"))
...
MCMCvis::MCMCtrace(out_metric$samples, params= c("surv", "transi_12", "transi_13", "transi_23"), pdf=T, ind = TRUE)
